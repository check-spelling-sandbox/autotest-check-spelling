...
sub get_artifacts {
    ...
    while (1) {
        ...
        if ($gh_err_text =~ /HTTP 404: Not Found/) {
            print "$program: The referenced repository ($repo) may not exist, perhaps you do not have permission to see it. If the repository is hosted by GHE, check-spelling does not know how to integrate with it.\n";
            exit 8;
        }
