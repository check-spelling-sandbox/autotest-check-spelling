    ...
    if ($results) {
        ...
        if (@missing_codes) {
            ...
            for my $missing_code (@missing_codes) {
                my $result_json = qq<{"ruleId": "$missing_rule_definition_id", "ruleIndex": $missing_rule_definition_index, $partialFingerprints "message": { "text": "$message" }, "locations": [ $locations_json_flat ] }>;
                my $result = decode_json $result_json;
                push @{$results}, $result;
            }
        }
        ...
        for my $result_index (0 .. scalar @{$results}) {
            ...
            my $code = $defined_codes{$result->{'ruleId'}};
            $code = $missing_rule_definition_index unless defined $code;
            $result->{'ruleIndex'} = $code;
        }
    }
