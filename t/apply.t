...
while ($state < 4) {
  `curl -s -v '$api_url' > '$expired_artifacts' 2> '$expired_artifacts_log'`;
  if (-s $expired_artifacts) {
    ...
    if ($?) {
      ...
      if ($expired_artifact =~ /^(\d+).*/) {
        $expired_artifact = $1;
        my $expired_artifact_url = `jq -r '.artifacts[] | select (.workflow_run.id==12194270619) | .url' '$expired_artifacts'`;
